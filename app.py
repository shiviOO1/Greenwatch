...
# Using weights parameter instead of deprecated pretrained parameter
try:
...
def submit():
    if request.method == 'POST':
        ...
        if image:
            ...
            # Find matching supplement info by index
            # The supplement_info CSV has an 'index' column that matches the prediction index
            supplement_row = suppliment_info[suppliment_info['index'] == pred]
            
            if not supplement_row.empty:
                supplement = supplement_row['supplement name'].values[0]
                supplement_img = supplement_row['supplement image'].values[0]
                supplement_prod_link = supplement_row['buy link'].values[0]
            else:
                # Fallback if no match found
                supplement = "No supplement information available"
                supplement_img = ""
                supplement_prod_link = "#"
